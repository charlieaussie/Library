...
class MySQLConnectionAbstract(object):
        ...
        for name in tls_ciphersuites:
            if "-" in name and name in ossl_cipher_suites_names:
                ...
                if name in TLS_CIPHER_SUITES["TLSv1.3"]:
                    translated_names[1].append(translated_name)
                else:
                    translated_names[0].append(translated_name)
            else:
                raise AttributeError(
    ...
    def config(self, **kwargs):
        ...
        try:
            self._consume_results = bool(config['consume_results'])
        except KeyError:
            self._consume_results = False
        ...
        if set_ssl_flag:
            ...
            elif set(('key', 'cert')) > set(self._ssl):
                self._ssl['key'] = None
                self._ssl['cert'] = None
    ...
    def set_login(self, username=None, password=None):
        ...
        the MySQL Server.
        """
        self._user = username.strip() if username is not None else ''

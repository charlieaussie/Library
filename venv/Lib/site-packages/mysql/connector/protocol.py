...
class MySQLProtocol(object):
    ...
    def _auth_response(self, client_flags, username, password, database,
        ...
        if not client_flags & ClientFlag.SECURE_CONNECTION:
            return plugin_auth_response + b'\x00'
        resplen = len(plugin_auth_response)
        return struct.pack('<B', resplen) + plugin_auth_response
    ...
    def parse_eof(self, packet):
        ...
        if unpacked[1] != 254 or len(packet) > 9:
            raise errors.InterfaceError(err_msg)
    ...
    def parse_statistics(self, packet, with_header=True):
        ...
        pairs = (
            packet[4:].split(b'\x20\x20')
            if with_header
            else packet.split(b'\x20\x20')
        )
        ...
        if parameters and data:
            ...
            for pos, _ in enumerate(parameters):
                ...
                if value is None:
                    ...
                    field_type = FieldType.BLOB if long_data_used[pos][0] else FieldType.STRING

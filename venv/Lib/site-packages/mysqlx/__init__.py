...
def _parse_address_list(path):
    ...
    path = path.replace(" ", "")
    array = (
        ("," in path or path.count(":") <= 1 or path.count("[") != 1)
        and path.startswith("[")
        and path.endswith("]")
    )
...
def _validate_settings(settings):
    ...
    if "ssl-ca" not in settings and settings.get("ssl-mode") in [
...
def _validate_connection_attributes(settings):
    ...
    if isinstance(conn_attrs, str):
        if conn_attrs == "":
            ...
            return
        if (
            not conn_attrs.startswith("[") or not conn_attrs.endswith("]")
        ...
        settings["connection-attributes"] = {} if conn_attrs else False
        return
